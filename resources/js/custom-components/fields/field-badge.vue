<template>
    <Fields :disabled="disabled" :label="label" :mode="mode" :name="name">
        <div v-if="Array.isArray(value)" class="inline-flex gap-1">
            <div
                v-for="item in value"
                :class="['badge', `bg-${item?.color} text-gray-500`]"
            >
                <Icon
                    class="h-3 w-3"
                    v-if="item?.icon"
        ...
        <div v-else class="inline-flex gap-1">
            <div :class="['badge', `bg-${color_from_value} text-gray-500`]">
                <Icon
                    class="h-3 w-3"
                    v-if="icon_from_value"
                    :color="color_from_value"
                    :name="icon_from_value"
                ></Icon>
...
.badge {
    @reference inline-flex gap-1 items-center text-xs justify-center px-2.5 py-0.5 rounded-md select-none;
